...
-(void)discoverBroadcastData:(BroadcastData *)data fromPeripheral:(CBPeripheral *)peripheral {
    if (_btUtil.state == CsScaleStateOpened || _btUtil.state == CsScaleStateBroadcast) {
        void (^block)() = ^{
            ...
            _kUIWeight.text = [NSString stringWithFormat:@"%g",(broadcastData.weight)];
        ...
        if (!broadcastData) {
            ...
            if ([broadcastData.mac isEqualToString:data.mac]) {
                broadcastData = data;
                block();
            }
        }
...
{
    [self.progressHud hide:YES];
    [MBProgressHUD showError:@"蓝牙连接断开,请检测设备是否正常" compleBlock:^{
