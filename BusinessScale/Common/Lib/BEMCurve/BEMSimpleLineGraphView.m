...
- (void)commonInit {
    ...
    _enableReferenceAxisFrame = NO;
    _enableBezierCurve = YES;
...
- (NSInteger)maxIndex
{
    ...
    for (i = 0; i<filteredArray.count; i++) {
        if ([filteredArray[i] isEqualToNumber:[self calculateMaximumPointValue]]) {
            break;
        }
    }
    return i;
...
- (NSInteger)minIndex
{
    ...
    NSInteger i = 0;
    for (i = 0; i<filteredArray.count; i++) {
        if ([filteredArray[i] isEqualToNumber:[self calculateMinimumPointValue]]) {
            break;
        }
    }
    return i;
